...
def display_filtered_table_results():
...
def display_cms_values(column):
    ...
    with column:
        ...
        st.session_state.cms_tt = st.number_input(label='CMS Total Time', value=st.session_state.cms_tt, placeholder=st.session_state.current_tt)
        st.session_state.cms_ist = st.number_input(label='CMS Intraservice Time', value=st.session_state.cms_ist, placeholder=st.session_state.current_ist)
        st.session_state.cms_work = st.number_input(label='CMS Work', value=st.session_state.cms_work, placeholder=st.session_state.current_work)
        st.session_state.cms_preservice = st.number_input(label='CMS Preservice', value=st.session_state.cms_preservice, placeholder=st.session_state.current_preservice)
        st.session_state.cms_postservice = st.number_input(label='CMS Postservice', value=st.session_state.cms_postservice, placeholder=st.session_state.current_postservice)
...
def display_potential_crosswalks():
    with st.container():
        ...
        st.dataframe(df)
        if st.session_state.potential_crosswalks is None:
            st.write(f'No potential crosswalks found for {st.session_state.hcpcs} with CMS work value of {st.session_state.cms_work}')
        else:
